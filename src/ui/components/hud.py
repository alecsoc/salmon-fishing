...
class Score():
  def __init__(self,x=10,y=10):
    ...
    self.text_surface = self.font.render(f"Score: {self.score}", True, self.color) #para renderizarlo por 1era vez
  def update_score(self,new_score):
      #recibe el puntaje actualizado
      self.score=new_score
      self.text_surface = self.font.render(f"Score: {self.score}", True, self.color) #basicamente renderizar el puntaje
